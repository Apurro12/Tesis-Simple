...
En este capitulo se van a aplicar las herramientas vistas en el capítulo anterior. 


Se va a obtener la energía de vacío del operador $A = - \partial ^2 _x$ sometido a distintas condiciones de contorno, para eso se va a calcular la función $ \zeta _A (s) $, primero exactamente, y luego en el último ejemplo, se utilizará el Heat-Kernel junto con dos métodos para aproximar $\zeta _A (s)$.
...
\begin{equation}
\begin{array}{c}
Res[ \zeta _A (s)] | _{s=1/2} = \frac{L \mu}{2 \pi} \\
Res[ \zeta _A (s)] | _{s=0} = 0 \\
Res[ \zeta _A (s)] | _{s=-1/2} = \frac{\gamma}{2 \pi \mu} \\
Res[ \zeta _A (s)] | _{s=-1} = 0 \\
\end{array}
\end{equation}
