...
Estos términos divergentes se pueden introducir en el lagrangiano, redefiniendo la acción efectiva de modo que resulte finita. $C_0$ renormaliza la constante cosmológica $\Lambda$, mientras que  $C_2$ renormaliza la masa $m$.
\begin{equation}
...
\begin{align}
\zeta  (s) &= \sum\limits_{n \in \mathbb N} \left( \frac{\lambda ^2 _n}{\mu }  \right) ^{-2s } \label{def.adim}\\[10pt]
...
En \ref{e0-zeta} no se introdujo $\mu$ explícitamente dado que $\zeta (-1/2)$ fue regular y esto no afecta el resultado final, a partir del próximo capítulo se utilizará la definición de la energía de vacío dada por \ref{eq.casimir.mu} en lugar de \ref{eq.casimir.no.mu}.