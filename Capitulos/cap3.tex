...
Utilizando (\ref{eq.aprox}) y tomando $S _1 = S _2 = 1$, $F _1 ^1 \left(1+  \frac{  \alpha}{2 i \lambda} ,2 ,2 i \lambda L  \right)$ se aproxima por
\begin{align}
\label{eq.completa}
...
&
    ...
    \, , 
\end{align}
aquí se puede utilizar $M( \lambda)$ para estudiar polos polos de $\zeta \left( - \frac{1}{2} \right)$. Para estudiar los polos de $\zeta (s)$ en la región  $Re(s) \geq - \frac{1}{2}$, o para estudiar la parte finita de $\zeta \left( - \frac{1}{2} \right)$ se deben tener en cuenta la forma completa de los términos $S _1$ y $S _2$.
...
\begin{equation}
	...
	1 - e ^{2 i \rho _n}
		\, ,
\end{equation}
de aquí puede que los ceros de $N ( \rho ) $ están dados por
\begin{align}
\label{eq.mu2}
...
\end{align}    
Dado que $\sum _{n=1} ^{\infty} O \left( n ^{-2s-3} \right)$ es regular en la región  $Re(s) > -1$, de la expresión anterior se ve que el polo en $s=  \frac{1}{2}$ está determinado por
...
\nonumber
	&| f(s) | < \infty , \, \forall s > -1
    	\, .
\end{align}
Aquí se encuentra el primer resultado de esta tesis. La existencia de un polo doble en $\zeta \left( -\frac{1}{2} \right)$ lo cual está en contradicción con el resultado (\ref{eq.ceros.zeta}), el cual determinaba que $\zeta (s)$ posee solamente polos simples. La función $f(s)$ será calculada en la sección \ref{sec.parte.finita.vacio}
...
En esta sección se utilizará el mismo procedimiento que en el capítulo \ref{sec.complejo}: se va a expresar la {\it función-$\zeta $} como una integral en el plano complejo.

Dado que se van a estudiar los polos en $s= \frac{1}{2}$ y $s=- \frac{1}{2}$, al igual que en la sección anterior se utilizará la función $M (z)$ definida en (\ref{eq.completa}). Así
...
\begin{equation}
    ...
    \frac{L e^{i \pi s} \mu ^{2s}}{2 \pi i} \frac{1}{s-1/2   }
    	\, ,
\end{equation}
ningún otro término aportará a este polo, entonces coincidiendo con el resultado (\ref{eq.res.2}) se obtiene para el polo en $s= \frac{1}{2}$
\begin{equation}
	\left(s- \frac{1}{2} \right)
    \zeta (s) \Big| _{s=1/2} = \frac{\mu L }{2 \pi} 
    	\, .
\end{equation}
De esta forma una vez calculado este término, el resto de la integral se puede reescribir de la forma
\begin{align}
...
\nonumber	
	...
	&| f(s) | < \infty , \, \forall s > -1
    	\, ,
\end{align}
obteniendo el mismo resultado que (\ref{eq.res.1}).
La función $f(s)$ será calculada en la sección \ref{sec.parte.finita.vacio}
...
Para controlar el orden del desarrollo de $ \frac{S' ( t)}{ S ( t)} $, el primer paso es desarrollar $\log S _1 (\lambda)$ alrededor de $\lambda \rightarrow \infty$, luego tomarle su derivada respecto de $\lambda$ y finalmente se evaluar en $\lambda = -i t$\\
\begin{equation}
\begin{aligned}
...
Utilizando la expresión de $\zeta  (s )$ dada en las secciones \ref{seq.2.com} y \ref{seq.2.asin} se obtiene para la energía de vacío
...
\, .
\end{equation}
Donde se puede apreciar que la energía de vacío depende del cutoff $\epsilon$. La función $f \left( - \frac{1}{2}\right)$ se calculará en la sección siguiente, conduciendo a la forma completa de la energía de vacío dada por las expresiones  (\ref{energia.vacio.final}) y (\ref{eq.vacio.completa.finita})
...
En esta sección se estudiará $ f \left( - \frac{1}{2} \right)$ de dos maneras diferentes: primero se utilizará el desarrollo asintótico correspondiente a la sección \ref{seq.2.asin} y la representación compleja tal como se hizo en la seccion  y \ref{seq.2.com}.
...
\, .
\end{align}
Así la función $f(s)$ definida en (\ref{eq.casimir.resultado}) queda determinado por todo lo que no depende de $\epsilon$ en la expresión anterior.
...
En el apéndice \ref{Apendice.1} se encuentra un script del software Mathematica que permite obtener los términos $a _p$, En este caso se calcularon hasta $p=10$. En la figura \ref{fig.finitas} está graficada la energía de vacío adimensionalizada obtenida por este método junto con el método que se describirá en la sección siguiente.
