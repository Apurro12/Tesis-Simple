...
Para calcular los siguienes polos, voy a volver a calcular la funcion $\zeta _A (s) $ con variable compleja, pero utilizando todos los terminos de la expancion en seríe de la funcion $M [\lambda]$

\begin{equation}
\begin{array}{c}
M[\lambda] =  
 \frac{e ^{ - \frac{i \alpha Log[2 \lambda L ]}{2 \lambda } } e ^{2 i \lambda L } }
 {\Gamma \left[ 1 - \frac{i \alpha}{2 \lambda}  \right] } S1 - 
 \frac{e ^{   \frac{i \alpha Log[2 \lambda L ]}{2 \lambda } } }
 	  {\Gamma \left[ 1 + \frac{i \alpha}{2 \lambda}  \right]  } S2 
\end{array}
\end{equation}

Al igual que antes voy a tener una parte exponencialmente creciente/decreciente, los Logaritmos quedan entonces:
...
Donde lo único que diferen de los calculados antes, en la suma de los terminos $S1$ y $S2$.

Sacando todos los terminos que no me contribuyen a los polos mas chicos que $s=-1/2$ obtengo:
...
\int _{1} ^{\infty} t ^{-2s } 
	...
	t ^{-2s-2} \left( \psi \left[ 1 - \frac{\alpha}{2 \lambda}\right] + \gamma \right) dt
...
De aquí se puede ver que no va a haber mas polos multiples, que el que habia en $s=-1/2$, porque las $S\S' $ son desarrollables en series de potencias.%, y las contribuciones a la parte finita en $s=-1/2$ estan dadas por el desarrollo en serie de la funcion Polygamma mas las derivadas logaritmicas de las correcciones asintoticas.
